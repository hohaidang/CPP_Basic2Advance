...
int main()
{
    ...
    for(int cycle = 0;cycle < cyclelimit;cycle++){
        if(newcustomer(min_per_cust)){
            if(line.isfull() && line1.isfull()) turnaways++;
            else{
                customers++;
                temp.set(cycle);
                if(line1.queuecount() < line.queuecount()) line1.enqueue(temp);
        ...
        sum_line += line.queuecount() + line1.queuecount();
    }
